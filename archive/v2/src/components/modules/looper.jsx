...
function looper({ id, audioContext, onRemove, deviceFile, rnbo, handleJackClick, updateCablePosition }) {
  const [rnboDevice, setRnboDevice] = useState(null);
  const [values, setValues] = useState({ loop: 0, rate: 1 })
  ...
  const [loopRange, setLoopRange] = useState({ min: 0, max: 1 });
    

  const [rate, setRate] = useState(1);
  const [rateRange, setRateRange] = useState({ min: -1, max: 5 });
  ...
  <Draggable 
    ...
    <div style={{ padding: '10px', border: '1px solid black', margin: '10px',  position: 'absolute'  }}>
      <p>looper</p>
        ...
        <div key={loop}>
          <label htmlFor="loop">loop: {values.loop}</label>
          <input
            type="range"
            id="loop"
            min={0}
            max={1}
            value={values.loop}
            onChange={(e) => handleParamChange("loop", e.target.value)}
          />
        </div>
      

        <div key={rate}>
          <label htmlFor="rate">rate: {values.rate}</label>
          <input
            type="range"
            id="rate"
            min={-1}
